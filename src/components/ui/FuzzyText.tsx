...
export default function FuzzyText({
    ...
    const init = async () => {
      ...
      const run = (timestamp: number) => {
        ...
        if (direction === 'horizontal') {
          // Shift each row left/right by a random fuzz offset.
          for (let j = 0; j < tightHeight; j++) {
            const dx = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            ctx.drawImage(offscreen, 0, j, offscreenWidth, 1, dx, j, offscreenWidth, 1);
          }
        } else if (direction === 'vertical') {
          // Shift each column up/down by a random fuzz offset.
          ...
          // Both: displace rows horizontally, then columns vertically.
          for (let j = 0; j < tightHeight; j++) {
            const dx = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            ctx.drawImage(offscreen, 0, j, offscreenWidth, 1, dx, j, offscreenWidth, 1);
          }
      ...
      const fuzzyCanvas = canvas as HTMLCanvasElement & { cleanupFuzzyText?: () => void };
      fuzzyCanvas.cleanupFuzzyText = cleanup;
    };
  ...
  return (
    <canvas
      ...
      className={className}
      role="img"
      aria-label={Children.toArray(children).join('')}
    />
  );
}
